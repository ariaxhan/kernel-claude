...
class SyncTests(unittest.TestCase):
    ...
    def test_manifest_aware_generated_state_machine(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            backup = repo / ".kernel-backups/adopt"
            self.assertEqual(0, run(sys.executable, str(SYNC), "adopt", str(repo),
                                    "--source", "CLAUDE.md", "--backup-dir", str(backup)).returncode)
            self.assertEqual("generated_current", self.audit_state(repo))
            (repo / "CLAUDE.md").write_text("changed\n")
            self.assertEqual("generated_stale", self.audit_state(repo))
            (repo / ".kernel-governance.json").unlink()
            (repo / "CLAUDE.md").write_text("rules\n")
            self.assertEqual("incomplete", self.audit_state(repo))
            (repo / "AGENTS.md").write_text("unrelated\n")
            self.assertEqual("conflict", self.audit_state(repo))

    def test_identical_native_pair_is_safely_adoptable(self):
        with tempfile.TemporaryDirectory() as td:
            repo = git_repo(Path(td) / "repo")
            ...
            result = run(sys.executable, str(SYNC), "adopt", str(repo),
                         "--source", "CLAUDE.md", "--backup-dir", str(backup))
            self.assertEqual(0, result.returncode, result.stderr)
            self.assertTrue((repo / "AGENTS.md").read_text().startswith("<!-- GENERATED by KERNEL"))
            self.assertEqual("same\n", (backup / "repo__CLAUDE.md").read_text())
            self.assertEqual("same\n", (backup / "repo__AGENTS.md").read_text())
            self.assertEqual("generated_current", self.audit_state(repo))

    def test_init_resumes_only_recognized_partial_state(self):
        for fail_after in ("1", "2"):
            with self.subTest(fail_after=fail_after), tempfile.TemporaryDirectory() as td:
                ...
                killed = run(sys.executable, str(SYNC), "init", str(repo),
                             "--backup-dir", str(repo / ".kernel-backups/init"), env=env)
                self.assertNotEqual(0, killed.returncode)
                resumed = run(sys.executable, str(SYNC), "init", str(repo),
                              "--backup-dir", str(repo / ".kernel-backups/init"))
                self.assertEqual(0, resumed.returncode, resumed.stderr)
                self.assertEqual("generated_current", self.audit_state(repo))
        with tempfile.TemporaryDirectory() as td:
            repo = git_repo(Path(td) / "repo")
            (repo / "CLAUDE.md").write_text("user rules\n")
            result = run(sys.executable, str(SYNC), "init", str(repo),
                         "--backup-dir", str(repo / ".kernel-backups/init"))
            self.assertNotEqual(0, result.returncode)
            self.assertFalse((repo / "AGENTS.md").exists())
            self.assertFalse((repo / ".kernel-governance.json").exists())

    def test_backup_directory_symlink_is_rejected_before_resolve(self):
        with tempfile.TemporaryDirectory() as td:
            repo = git_repo(Path(td) / "repo")
            (repo / "CLAUDE.md").write_text("rules\n")
            real = repo / "real-backups"
            real.mkdir()
            link = repo / "backup-link"
            link.symlink_to(real, target_is_directory=True)
            result = run(sys.executable, str(SYNC), "adopt", str(repo),
                         "--source", "CLAUDE.md", "--backup-dir", str(link))
            self.assertNotEqual(0, result.returncode)
    ...
    def test_audit_classifies_and_deduplicates_worktree(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(1, data["counts"]["conflict"])
    ...
    def test_interrupted_adoption_is_auditable_and_rerun_repairs(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual("incomplete", json.loads(audit.stdout)["repositories"][0]["state"])
            self.assertEqual(0, run(sys.executable, str(SYNC), "adopt", str(repo),
                                    "--source", "CLAUDE.md",
