...
run_test() {
  ...
  if [ "$QUICK" = 1 ]; then
    local slow
    for slow in "${QUICK_SKIP[@]}"; do [ "$slow" = "$name" ] && return 0; done
  fi
...
# --quick skips these: >=0.4s each in a full timed run (2026-09-12).
QUICK_SKIP=(
  "learn pattern shows in read-start"
  "write-end creates checkpoint"
  "read-start with data"
  "status shows counts"
  "prune keeps N"
  "preflight accepts append-only history growth"
  "recent shows checkpoints"
  "recall defaults to pure FTS (embed opt-in)"
  "recall alias expansion (migration 017)"
  "recall relevance floor (bm25, default on)"
  "recall ids survive sentinel-in-content"
  "unicode content"
  "long content"
  "session-start outputs KERNEL"
  "session-start keeps identity outside target"
  "session-start creates agent file"
  "session-start shows checkpoint after compact"
  "JSON with quotes safe"
  "no shell expansion"
  "checkpoint has timestamp"
  "DB size reasonable"
  "complexity runs through npm verify"
  "graph-project from receipt"
  "graph-project idempotent"
  "graph outcome from write-end"
  "agentdb health runs"
  "agentdb metrics shows learnings"
  "learning dedup reinforces existing"
  "learning dedup requires same type"
  "preflight restores dropped migration table"
  "preflight idempotent after table drift"
  "nested project retains KERNEL compaction fallback"
  "Vaults root SessionStart keeps governance without restore"
  "post-compact-restore fast exit without marker"
  "inject-context surgeon outputs gotchas"
  "read-start outputs Known Gotchas section"
  "read-start --lean is minimal (8.4.0)"
  "session-start gives concrete recall recipe"
  "read-start full keeps weighted + tail"
  "graph build creates edges (016)"
  "graph promote finds a cluster"
  "graph promote empty when no cluster"
  "graph edges excluded from mirror"
  "read-start bumps load_count not hit_count"
  "Kernel 9 router, packs, and host adapters"
  "violation corpus: gates still refuse, and fail as declared"
  "recall dedups identical insights"
  "recall hides human_only learnings"
  "recall bumps hit_count on surfaced rows"
  "recall --global unions + tags global hits"
  "recall --global graceful when global absent"
  "recall --global never leaks human_only"
  "recall survives sqlite control-char escaping"
  "embed-sync writes vectors"
  "recall --ids is side-effect-free"
  "recall --ids prints only ids"
  "hybrid never regresses on fixture (recall@5)"
  "recall FTS-only when no vectors (degradation)"
  "learn --check passes, then fails when the failure returns"
  "reinforcing a learning attaches --check"
  "generated governance adapters and operator"
  "7.23 links repair and data is unchanged"
  "rollback tool selects a lower local runtime"
  "rollback tool accepts real legacy common"
  "init AgentDB targets selected Vaults"
  "latest finds newest manifest"
  "latest uses identity timestamp"
  "created timestamp is strict RFC3339"
  "git_diff rejects option injection"
  "CLI rejects bad options cleanly"
  "deactivate rejects malformed ledger transactionally"
  "deactivate projection retry merges once"
  "deactivate rewrites receipt once"
  "init is idempotent"
  "learn failure"
  "verdict pass"
  "prune all compacts free pages"
  "query works"
  "error records tool errors"
  "special chars (SQL injection)"
  "newlines in content safe"
  "export creates file"
  "graph-suggest shadow mode"
  "SQL injection via prune"
  "agentdb emit with duration"
  "agentdb metrics runs"
  "agentdb metrics custom days"
  "post-compact-restore outputs marker content"
  "post-compact-restore deletes marker"
  "inject-context adversary outputs failures"
  "agentdb trace records trace"
  "agentdb antibody searches learnings"
  "decay spares loaded (load_count>0) learnings"
  "learn auto-populates domain from PWD"
  "authority monotonic with explicit rollback"
  "startup arms reconciliation"
  "manifest paths stay in repo"
  "activate/deactivate roundtrip"
  "migration: no live command references"
  "complexity ratchet holds"
)
QUICK=0
