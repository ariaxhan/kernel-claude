...
11. **`--debug` flag**: run `claude --debug` to expose raw tool inputs/outputs and internal state transitions — deeper than `--verbose`, which shows only high-level steps. Use when `--verbose` doesn't pinpoint which tool call produced the unexpected state. The basic quartet for hard bugs: `--debug`, `--verbose`, `/cost`, `/compact`. <!-- Updated 2026-06-28: https://institute.sfeir.com/en/claude-code/claude-code-advanced-best-practices/debugging/ -->
