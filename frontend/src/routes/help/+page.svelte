...
The tier is determined by counting how many files need to change. When in doubt, KERNEL assumes higher tier (more safety).`,
    ...
    },
    {
      ...
      items: [
        ...
        { name: '/kernel:debug', desc: 'Diagnosis before prescription. Reproduce, hypothesize, isolate with binary search, fix the root cause; refactor mode maps the blast radius.' },
